...
def build_unique_examples_of_iteration(examples):
    """ Make examples unique by selecting one with maximum value (v) for the same positions. 
        If there are several samples with maximum value then average policy_vector (pi) for all of them.
    """
    validate_random_sample(examples)
    stateMap = {}
    # collect samples with maximum reward for the same positions
    for (position, policy_vector, reward, s) in examples:
        if not s in stateMap:
            stateMap[s] = deque()
            stateMap[s].append((position, policy_vector, reward, s))
        else:
            if reward == stateMap[s][0][2]:
                stateMap[s].append((position, policy_vector, reward, s))
            elif reward > stateMap[s][0][2]:
                stateMap[s] = deque()
                stateMap[s].append((position, policy_vector, reward, s))
